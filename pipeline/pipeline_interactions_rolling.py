...
class FeatureBuilder:
    def __init__(self, smoothing_factor=0.3, window_size=5):
    ...
    def bbox_to_features(self, boxes, ids, frame_shape):
        ...
        for box, tid in zip(boxes, ids):
            ...
            if tid not in self.rolling_history:
                self.rolling_history[tid] = deque(maxlen=self.window_size)

            self.rolling_history[tid].append([speed, ax, ay])
            history_array = np.array(self.rolling_history[tid])
            roll_mean = np.mean(history_array, axis=0)
            roll_std = np.std(history_array, axis=0)

            base_feats.append([
                cx, cy, bw, bh, vx, vy, speed, ax, ay,
                roll_mean[0], roll_mean[1], roll_mean[2],
                roll_std[0], roll_std[1], roll_std[2]
            ])
